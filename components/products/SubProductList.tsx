    ...
    <div style={{ marginTop: "48px" }}>
      {Array.from(grouped.entries()).map(([rangeKey, items]) => (
        <div key={rangeKey ?? "default"} style={{ marginBottom: "40px" }}>
          {rangeKey && hasRanges && (
            <p style={{ fontSize: "12px", fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--cd-orange)", marginBottom: "16px", paddingBottom: "8px", borderBottom: "1px solid var(--cd-line)" }}>
              {t(rangeKey as Parameters<typeof t>[0])}
            </p>
          )}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "1px", border: "1px solid var(--cd-line)", backgroundColor: "var(--cd-line)" }}>
            {items.map((sp) => (
              <div key={sp.id} style={{ backgroundColor: "var(--cd-white)" }}>
                {sp.image ? (
                  <div style={{ backgroundColor: "var(--cd-white)", borderBottom: "1px solid var(--cd-line)", height: "240px", overflow: "hidden" }}>
                    <img
                      src={sp.image}
                ...
                <div style={{ padding: "24px" }}>
                  <p style={{ fontWeight: 600, fontSize: "15px", color: "var(--cd-black)", marginBottom: "8px", lineHeight: 1.3 }}>
                  ...
                  {sp.brand && (
                    <p style={{ marginTop: "12px", fontSize: "11px", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--cd-gray-text)" }}>
                      {sp.brand}
                    </p>
                  )}
                </div>
